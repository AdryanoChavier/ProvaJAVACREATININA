...
public class Creatinina {
	...
	medico daoMed = new medico();
	
	
	public Creatinina() throws SQLException{
	...
	public List<Creatininaa> getLista() throws SQLException{
		...
		List<Creatininaa> exames = new ArrayList<Creatininaa>();
		...
		while (rset.next()) {
			...
			Creatininaa exame = new Creatininaa(rset.getInt("a.id"), rset.getString("a.resultado"), paciente, medico);
			exames.add(exame);
	...
	public Creatininaa getAcidoUricobyId(int id) throws SQLException {
		String sql = "SELECT a.id, a.resultado, m.id, m.nome, m.cpf, m.especialidade,"
				+ "p.id, p.nome, p.cpf FROM acido_urico a "
				+ "JOIN medico m ON a.id_medico = m.id "
				+ "JOIN paciente p ON a.id_paciente = p.id";
	...
	public void atualiza(int id, String resultado, int id_paciente, int id_medico , String chave) throws Exception {
		...
		Creatininaa ac = getAcidoUricobyId(id);
